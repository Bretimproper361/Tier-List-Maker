...
When handling massive tier lists (1000+ images), some slowdown is expected when importing that many images and when exporting the JSON (~20-30 seconds), and there can be slight slowdown when moving items around. You may also have limitations from your operating system (or browser or file explorer) that limit how many images can be imported at once, a workaround is breaking large amounts of images into groups (like 300-500 images each), and importing them one batch at a time into the list.
